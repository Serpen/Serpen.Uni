...
namespace Serpen.Uni.Automat.Turing {
    public class TuringMachineMultiTrack : TuringMachineBase<TuringTransformMultiTrack.TuringKey, TuringTransformMultiTrack.TuringVal> {
        ...
        public override bool AcceptWord(string w) {
            ...
            Uni.Utils.DebugMessage($"w: {w}=>{string.Join(',', wordTracks)}", this, Uni.Utils.eDebugLogLevel.Verbose);
        ...
        public override string GetBandOutput(string w) {
            ...
            while (tcfg != null && !IsAcceptedState(tcfg.State)) {
                ...
                if (runs > MAX_TURING_RUNS)
                    throw new TuringCycleException($"possible Turing cycle at {runs} with {w} now is: {lastBand.Trim(BlankSymbol)}", this);
                runs++;
            }
        ...
        public override IAutomat PurgeStates() {
            ...
            return new TuringMachineMultiTrack($"{Name}_purged", Tracks, names, Alphabet, BandAlphabet, newT, translate.ArrayIndex(StartState), BlankSymbol, aStates);
        }
