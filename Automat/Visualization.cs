...
namespace Serpen.Uni.Automat {
    public static class Visualization {
        ...
        static Pen PEN_START_ARROW { get {
            const int WIDTH = 2;
        ...
        public static Bitmap DrawAutomat(IAutomat A) {
            ...
            if (A is ContextFree.PDA pda) {
                foreach (var t in pda.Transform)
                {
                    foreach (var v in t.Value)
                    {
                        desc = $"{(t.Key.ci.HasValue ? t.Key.ci.Value : Utils.EPSILON)}|{(t.Key.cw.HasValue ? t.Key.cw.Value : Utils.EPSILON)}->{(!string.IsNullOrEmpty(v.cw2) ? v.cw2 : Utils.EPSILON.ToString())}";
                        qStart = (int)t.Key.q;
                        qEnd = (int)v.qNext;
                        DrawTransformLine(ref g, qStart, qEnd, desc, vCenter, ref lastCurveHeigth, ref alreadyDrawn);
                    }

                }
            } else if (A is ContextFree.DPDA dpda) {
                foreach (var t in dpda.Transform)
                {
                    {
                        desc = $"{(t.Key.ci.HasValue ? t.Key.ci.Value : Utils.EPSILON)}|{(t.Key.cw.HasValue ? t.Key.cw.Value : Utils.EPSILON)}->{(!string.IsNullOrEmpty(t.Value.cw2) ? t.Value.cw2 : Utils.EPSILON.ToString())}";
                        qStart = (int)t.Key.q;
                        qEnd = (int)t.Value.qNext;

                        DrawTransformLine(ref g, qStart, qEnd, desc, vCenter, ref lastCurveHeigth, ref alreadyDrawn);
                    }

                }
            } else if (A is Finite.FABase fa) {
                foreach (var t in fa.Transform)
                {
                    foreach (uint v in t.Value)
                    {
                        desc = t.Key.c.ToString();
                        if (desc == "")
